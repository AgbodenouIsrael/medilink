...
<head>
    ...
    <style>
        ...
        @media (max-width: 768px) {
            ...
            .auth-image, .auth-form-container { width: 100%; }
            .auth-image { padding: 20px; }
...
<body>
    ...
    <div class="auth-container">
        <div class="auth-image">
            ...
            <p>Rejoignez le réseau Medilink pour gérer vos stocks et recevoir les ordonnances numériques instantanément.</p>
        </div>
        ...
        <div class="auth-form-container">
            ...
            <form action="{{ route('dashboard_pharmacie') }}" method="POST" novalidate>
                @csrf
                <div class="form-grid">
                    <div class="input-group full-width">
                        <label for="pharma_name">Nom de l'Officine</label>
                        <input type="text" id="pharma_name" name="nom_officine" placeholder="Ex: Pharmacie de la Gare" value="{{ old('nom_officine') }}" required>
                        @error('nom_officine') <span class="error-msg">{{ $message }}</span> @enderror
                    </div>

                    <div class="input-group">
                        <label for="pharmacist_name">Pharmacien Titulaire</label>
                        <input type="text" id="pharmacist_name" name="pharmacien_titulaire" placeholder="Dr. Nom Prénom" value="{{ old('pharmacien_titulaire') }}" required>
                        @error('pharmacien_titulaire') <span class="error-msg">{{ $message }}</span> @enderror
                    </div>

                    <div class="input-group">
                        <label for="license_id">N° Licence / Agrément</label>
                        <input type="text" id="license_id" name="numero_licence" placeholder="Ex: PH-123456" value="{{ old('numero_licence') }}" required>
                        @error('numero_licence') <span class="error-msg">{{ $message }}</span> @enderror
                    </div>

                    <div class="input-group full-width">
                        <label for="address">Adresse Complète</label>
                        <input type="text" id="address" name="adresse_complete" placeholder="Rue, Ville, Code Postal" value="{{ old('adresse_complete') }}" required>
                        @error('adresse_complete') <span class="error-msg">{{ $message }}</span> @enderror
                    </div>

                    <div class="input-group">
                        <label for="email">Email Professionnel</label>
                        <input type="email" id="email" name="email" placeholder="contact@pharmacie.com" value="{{ old('email') }}" required>
                        @error('email') <span class="error-msg">{{ $message }}</span> @enderror
                    </div>

                    <div class="input-group">
                        <label for="phone">Téléphone</label>
                        <input type="tel" id="phone" name="telephone" placeholder="+228..." value="{{ old('telephone') }}" required>
                ...
                <div class="input-group full-width" style="margin-top: 10px;">
                    <label style="display: flex; align-items: center; gap: 10px; cursor: pointer;">
                        ...
                        <span style="font-size: 0.9em;">J'atteste être un pharmacien agréé et j'accepte les <a href="#" style="color:var(--color-pharma);">CGU</a>.</span>
                    </label>
                    @error('agree_terms') <span class="error-msg">{{ $message }}</span> @enderror
                </div>
            ...
            <div class="auth-footer">
                ...
                <p style="margin-top: 10px;"><a href="{{ route('login') }}"><i class="fas fa-arrow-left"></i> Retour à l'accueil</a></p>
            </div>
        </div>
    </div>
