...
@section('title', 'Mon Dossier Médical - Medilink')
...
@section('content')
    @php
        // Fonction helper pour formater les dates (si nécessaire localement)
        ...
        // Les collections sont passées par le contrôleur, mais on garde des fallbacks au cas où
        $antecedents = $antecedents ?? collect();
        $allergies = $allergies ?? collect();
        $ordonnances = $ordonnances ?? collect();
        $diagnostiques = $diagnostiques ?? collect();
        $traitements = $traitements ?? collect();
        $documents = $documents ?? collect();
    @endphp

    <header class="dashboard-header">
        <h2>Mon Dossier Médical</h2>
        <p>Bienvenue, {{ Auth::guard('patient')->user()->prenom }}, gérez vos informations de santé en temps réel.</p>
    </header>
    ...
    <section class="medical-record-sections">
        ...
        <div class="medical-section">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <a href="{{ route('profil') }}"><button id="openModal" class="btn-add"><i class="fas fa-edit"></i> Modifier
                        le profil</button></a>
            </div>
            <div class="medical-info-grid">
                <div class="info-item"><label>Nom Complet</label>
                    <p>{{ Auth::guard('patient')->user()->prenom }} {{ Auth::guard('patient')->user()->nom }}</p>
                </div>
                <div class="info-item"><label>Date de Naissance</label>
                    <p>{{ formatDate(Auth::guard('patient')->user()->date_naissance) }}</p>
                </div>
                <div class="info-item"><label>Genre</label>
                    <p>{{ Auth::guard('patient')->user()->genre ?? 'Non renseigné' }}</p>
                </div>
                <div class="info-item"><label>Contact</label>
                    <p>{{ Auth::guard('patient')->user()->contact ?? 'Non renseigné' }}</p>
                </div>
                <div class="info-item"><label>Email</label>
                    <p>{{ Auth::guard('patient')->user()->email }}</p>
                </div>
                <div class="info-item"><label>Adresse</label>
                    <p>{{ Auth::guard('patient')->user()->adresse ?? 'Non renseigné' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="medical-section">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <button onclick="openModal('antecedent')" class="btn-add"><i class="fas fa-plus"></i> Ajouter un
                    antécédent</button>
            </div>
                                    ...
                                    <td>
                                        ...
                                        <form action="{{ route('antecedent.destroy', $antecedent->id) }}" method="POST"
                                            ...
                                            <button type="submit" class="btn-action btn-delete"
                                                onclick="return confirm('Supprimer cet antécédent ?')">
        ...
        <div class="medical-section">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <button onclick="openModal('allergie')" class="btn-add"><i class="fas fa-plus"></i> Ajouter une
                    allergie</button>
            </div>
                        ...
                        <tbody>
                            @foreach($allergies as $allergie)
                                <tr>
                                    ...
                                    <td>
                                        <form action="{{ route('allergie.destroy', $allergie->id) }}" method="POST"
                                            style="display: inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn-action btn-delete"
        ...
        <div class="medical-section">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <button onclick="openModal('document')" class="btn-add"><i class="fas fa-upload"></i> Importer un
                    document</button>
            </div>
            ...
            <form action="{{ route('antecedent.store') }}" method="POST">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="commentaires" id="commentaires" rows="2"
            ...
            <form action="{{ route('allergie.store') }}" method="POST">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" name="nom_allergie" id="nom_allergie" required
                    ...
                    <div class="form-group full-width">
                        ...
                        <textarea name="symptomes" id="symptomes" rows="2"
                            placeholder="Décrivez les symptômes..."></textarea>
                    </div>
                    <div class="form-group full-width">
                        <label for="traitement_urgence">Traitement d'urgence</label>
                        <textarea name="traitement_urgence" id="traitement_urgence" rows="2"
                            placeholder="Que faire en cas de réaction..."></textarea>
                    </div>
                </div>
                <div class="modal-footer">
...
@section('scripts')
    <script>
        ...
        if (editModal && openProfileBtn) {
            openProfileBtn.onclick = () => editModal.style.display = "block";
        }

        if (editModal && closeProfileBtn) {
